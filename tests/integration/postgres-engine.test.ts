...
let engine: PostgresQueryEngine;
let collection = "";
...
function documentsTableName(): string {
  return `${quoteIdentifier(schemaName)}.${quoteIdentifier("nosql_odm_documents")}`;
}
...
function createSqlIdentifier(prefix: string): string {
  const raw = createTestResourceName(prefix)
    .toLowerCase()
    .replace(/[^a-z0-9_]/g, "_");
  ...
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      if (i === attempts - 1) {
        ...
        throw new Error(
          `Unable to connect to Postgres at ${host}:${String(port)} (db=${database}). ` +
            `Start it with \`bun run services:up:postgres\`. ` +
...
describe("postgresEngine integration", () => {
  beforeAll(async () => {
    pool = new Pool({
      host,
      port,
    ...
    await requirePool().query(`CREATE SCHEMA IF NOT EXISTS ${quoteIdentifier(schemaName)}`);
  });
  ...
  test("supports custom internal table names", async () => {
    ...
    const rawDoc = await requirePool().query(
      `
        SELECT doc_key
        FROM ${quoteIdentifier(schemaName)}.${quoteIdentifier("custom_documents")}
  ...
  test("get throws for invalid stored document payload", async () => {
    ...
    await requirePool().query(
      `
        INSERT INTO ${documentsTableName()} (collection, doc_key, doc_json)
        VALUES ($1, $2, $3::jsonb)
      `,
  ...
  test("loadCheckpoint throws for invalid checkpoint row", async () => {
    const checkpointsTable = createSqlIdentifier("pg_checkpoint_invalid");
    const customEngine = postgresEngine({
      client: requirePool(),
      schema: schemaName,
    ...
    await requirePool().query(
      `
        ...
        USING to_jsonb(cursor)
      `,
    );

    await requirePool().query(
      `
        INSERT INTO ${qualifiedTableName(checkpointsTable)} (collection, cursor)
        VALUES ($1, $2::jsonb)
      `,
      [collection, "123"],
    );
  ...
  test("getStatus throws for invalid lock row", async () => {
    const locksTable = createSqlIdentifier("pg_lock_status_invalid");
    const checkpointsTable = createSqlIdentifier("pg_checkpoint_status_valid");
    const customEngine = postgresEngine({
      client: requirePool(),
      schema: schemaName,
    ...
    await requirePool().query(
      `
        ...
        ALTER COLUMN acquired_at TYPE TEXT
        USING acquired_at::text
      `,
    );
  ...
  test("getStatus throws for invalid checkpoint row", async () => {
    const locksTable = createSqlIdentifier("pg_lock_status_valid");
    const checkpointsTable = createSqlIdentifier("pg_checkpoint_status_invalid");
    const customEngine = postgresEngine({
      client: requirePool(),
      schema: schemaName,
      migrationLocksTable: locksTable,
  ...
  test("acquireLock throws for invalid existing lock row", async () => {
    const locksTable = createSqlIdentifier("pg_lock_acquire_invalid");
    const checkpointsTable = createSqlIdentifier("pg_checkpoint_acquire_valid");
    const customEngine = postgresEngine({
      client: requirePool(),
      schema: schemaName,
      migrationLocksTable: locksTable,
